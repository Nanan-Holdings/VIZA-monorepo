...
import type { ChatMessage, ToolItineraryPart } from "@/lib/types";
...
function extractToolItinerary(messages: ChatMessage[]): ItineraryDay[] {
  for (let i = messages.length - 1; i >= 0; i -= 1) {
    const message = messages[i];
    if (message.role !== "assistant") continue;

    const toolPart = message.parts?.find(
      (part) => part.type === "tool-itinerary"
    ) as ToolItineraryPart | undefined;

    if (!toolPart?.output?.length) continue;
...
function buildTravelPayloadFromChat(messages: ChatMessage[]) {
