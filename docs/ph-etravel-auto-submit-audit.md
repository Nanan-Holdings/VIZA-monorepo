...
- **Sex 不是原生下拉,是隐藏字段 + react-select 组合**:真实字段名是 `gender`(不是 VIZA 现在用的 `sex`),可见的是一个 `react-select-3-input` 搜索框,实际值写入旁边的 `<input type=hidden name="gender">`。**已现场点开验证**,真实选项只有两个:`FEMALE`/`MALE`(大写英文,不是 `PH_ETRAVEL_SEX_OPTIONS` 种子里的 `MALE`/`FEMALE` 顺序或大小写猜测——大小写这次是对的,但仅有两项已确认无误)。Playwright 不能用 `selectOption`,要走"点开→点选项文字"这条路。
- **Suffix 也是 react-select(`react-select-2-input`,不是 3——我最初读错了两个 select 的对应关系,现场点开验证后已更正),隐藏字段是 `extension_name`**。**已现场点开拿到完整 21 项真实选项**:`JR, SR, II, III, IV, V, VI, VII, VIII, IX, X, XI, XII, XIII, XIV, XV, XVI, XVII, XVIII, XIX, XX`——比 VIZA 现在完全没有建模这个字段更进一步,确认了真实枚举值(注:这是下拉里显示的文字,真实提交值大概率就是这几个字符串本身,但没有实际选中提交去反查,留一个"大概率但未 100% 确认"的标记)。
- **国籍/出生国家/护照签发国/职业,都是同一种自定义 combobox(HeadlessUI Combobox),不是原生 `<select>`**:真实字段名 `nationality_country_code`/`country_of_birth_code`/`passport_issued_country_code`/`occupation_code`——都是"code"后缀,确认是编码字典,不是自由文本(**`occupation_code` 这一条是新发现:VIZA 现在把 occupation 当成自由文本字段,官网真实是一个编码下拉**)。这几个 combobox 的 DOM id 是 `headlessui-combobox-input-:r1:` 这种自动生成、随渲染次数变化的 id,**不能拿 id 做选择器**,要靠前面的 label 文字或容器结构定位。**已现场验证交互方式**:这是一个"客户端本地过滤"的搜索框(不是每次按键都请求后端接口)——聚焦后不立即展开全部选项,输入文字后大约 500-800ms 有一个防抖延迟,然后从一份本地已加载的完整名单里过滤(`nationality_country_code` 输完整份列表有 250 项,是**国籍形容词**如"Filipino"/"Afghan"/"Chinese",不是国家名"China"——真要按国家名去填会填不进去,必须用形容词形式的清单,选择器/filler 要按这份清单的英文形容词拼写去匹配,不能直接用 ISO 国家名字符串)。
...
- **`passport_number`/`birth_date`/`passport_issued_date` 是原生 text input**,不是自定义组件。**已现场验证 `birth_date`:不是只读的**(`readOnly:false`,没有 `readonly` 属性)——跟台湾那个"日期输入框全部只读,`.fill()`静默失败"的坑**不一样**,这里的日期控件是 `react-datepicker` 库(不是 TW 的 jQuery UI Datepicker),输入框本身可以直接输入文字(格式 `MM/DD/YYYY`,点开会有一个弹出日历辅助选择,但文本框不是只读的)。这是一个好消息,但严谨起见还没有实测过`.fill()`真的能不能让 react-datepicker 正确识别/触发它的 onChange(有些 react-datepicker 封装即使 input 不是 readonly,也需要真实键盘事件才能触发内部状态更新,单纯改 value 属性可能不生效)——真正写 `fillers.ts` 时建议还是先跑一次实测,不要直接假设`.fill()`一定成功。
