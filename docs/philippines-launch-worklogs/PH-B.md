...
## Production DB migration attempt blocked（2026-08-18）

- Tommy approved applying `0149_runner_country_claim.sql` then `0150_ph_etravel_submission_state_sync.sql` to production. Preflight confirmed current branch `integration/product-work-20260813` at HEAD `5132dabccb060f5990e00950060ab351969fdeb0`.
- Target identification succeeded: repo docs and current deployed envs identify production Supabase as `viza-production` / project ref `oyjxdzsoejraedqghndi`; agent-backend `DATABASE_URL` points to `db.oyjxdzsoejraedqghndi.supabase.co`, and all checked Supabase URLs use the same ref. No staging/local ref was used.
- Migration did **not** run. Supabase CLI is not installed in this environment, and the only configured Postgres URL uses the direct DB host `db.oyjxdzsoejraedqghndi.supabase.co`. That host exposes only an IPv6 DNS answer here, while the local Node/Python connection path failed with `getaddrinfo ENOTFOUND db.oyjxdzsoejraedqghndi.supabase.co`. No pooler URL is configured in repo env files.
- No seed, application creation, runner claim, business-row read, official-site action, final Submit, deployment, or secret output occurred. The failed script stopped before opening any DB transaction or executing migration SQL.
- Current status: `0149` / `0150` production application is **blocked / not verified** from this workstation. Required next step is a reliable production DB execution path for project `oyjxdzsoejraedqghndi`, preferably Supabase CLI authenticated to that project or an official Supabase pooler/direct connection string whose host resolves from this environment. Once available, apply only `0149` then `0150`, then run the existing read-only function/index/privilege verification.

## Production migration MCP preflight correction（2026-08-18）
