...
- Production redeploy：commit `b88842fe` deployed to `dpl_ucW1x4QwcQ1kFPGAYpm6ca6xWULH` / `https://viza-internal-6eaniibgz-viza-gmail-s-projects.vercel.app`，alias `https://app.viza.it.com` READY。Post-deploy smoke：无效 synthetic password 请求 7.17s/1.93s 返回普通 auth error，invalid synthetic OTP request 2.26s 返回普通 auth error，均不再返回 `provider_unavailable`；PH login URL 保留 `next=%2Fclient%2Farrival-cards%2Fphilippines`。未提交用户邮箱、未读取 OTP/Cookie/token。
