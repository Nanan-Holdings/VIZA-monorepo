...
- 美国和法国预约助手的活跃 job 每 7 秒读取状态，原先可见性恢复会
  在未完成请求之外再启动 GET。一次状态接口又会读取 job、slots、
  manual actions、confirmation 等多项数据，重复 GET 会放大后端读取。
- 两个轮询 effect 增加 `inFlight` 防重入，每次轮询独立持有
  `AbortController`。卸载或 effect 失效会取消该轮询请求，晚到结果
  不更新快照、不重新排程；完成后仍按原有 7 秒周期继续。隐藏页面
  暂停、30 秒隐藏检查和原有终态集合保持不变。
- 两个只读 status helper 增加可选 `AbortSignal` 并传递到 fetch，
  保留初始加载和显式操作的原有调用方式。没有更改用户确认、预约创建、
  选位、付款、官方提交或任何 POST，也没有更改 UI、状态映射或依赖。
- 这是每个活跃轮询 effect 的请求限制；初始读取、用户显式操作和其他
  页面的请求不共享该限制。浏览器取消不等于已保证后端数据库任务立即
  终止，本轮未修改后端取消机制或新增请求超时。

### 第二十轮本地验证

- 美国预约组件 8 项测试覆盖慢读取、重复可见性事件、7 秒节奏、隐藏
  暂停、失败重试、卸载取消、无 job 和原有终态。测试同时断言挂载和
  轮询没有调用创建、运行、选位、付款确认等变更 API。
- 法国预约轮询新增 5 项、既有 assisted-live 回归 4 项通过；另有
...
- 本地 Next 使用 loopback 服务配置，浏览器访问合成 application ID
  的美国及法国预约入口，均正常重定向登录页，表单可见且无未处理异常。
  临时服务器与标签页已关闭。缺少独立认证环境，真实登录后的预约卡
  和官方门户没有用于本轮验证，状态行为通过合成组件测试覆盖。
- 发布前固定使用 Vercel CLI 59.14.0，确认组织身份为
...
## 2026-09-10：生产被动观察与验收工具复核

读取已有 GitHub Actions 运行
[`34438984197`](https://github.com/Nanan-Holdings/VIZA-monorepo/actions/runs/34438984197)
的精确名称 artifact `passive-production-capacity-34438984197`，未触发新任务
或向生产发送压测请求。该 artifact 未过期，创建时间为
`2026-09-10T04:55:28Z`；本地只读 report 校验成功，原始文件保存在忽略目录
`.dev-logs/capacity-followup-34438984197/`。

### 生产观察结果

窗口从 `2026-09-09T04:53:41.474Z` 到 `2026-09-10T04:55:00.349Z`，
共 5 次观察、跨度 24.02 小时，满足现有趋势完整性门槛。每次观察仅包含
十秒内的三个样本；这不是连续 24 小时监控，也不是高并发实验。

| 聚合指标 | 结果 |
| --- | --- |
| 采样连接使用率峰值 / 中位数 | 17.54% / 15.79% |
| 总连接数峰值 / 活跃连接峰值 | 10 / 1 |
| 未授予锁峰值 | 0 |
| 超过 30 秒的事务 / idle-in-transaction 峰值 | 0 / 0 |
| 采样区间新增死锁 / 回滚 | 0 / 0 |
| 阻塞项 | 0 |
| 报告状态 | `warn`；数据库项目 marker 缺失，Management API 身份校验仍有效 |

这些数据未显示采样时数据库饱和，但采样活跃连接很少，不能外推为能够
承载 100 个真实登录用户。保留 `warn`，不为让报告变绿而修改生产标记。

完整窗口还提供了后续查询复核线索：`runner_job` 顺序扫描增加 17,144 次，
但每次平均仅读取 1.04 个 tuple；单凭扫描次数不足以证明缺索引。
`portal_health` 和 `portal_health_checks` 各有 8,352 次写计数增量，
`status_incidents` 有 2,346 次。统计趋势仅用于定位需要检查的轮询和写入路径，
本次没有据此添加索引、改变探测频率或修改数据库。

### 验收执行环境修正

发现手动 `online-capacity-gate.yml` 仍使用 Node.js 20，而 agent-backend
的 package engine 和正常 CI 已要求 Node.js 24。将容量验收工作流更新为
Node.js 24，避免用与受支持运行时不一致的环境执行后续验收；保护环境、
精确提交检查、非生产标记、请求范围和密钥隔离保持原有要求。

同时复核发现，认证场景允许手动设置 10 秒 pacing，但原发布评估没有接收
该参数。同样的用户数和持续时间下，这会减少请求量，仍可能被判为通过。
认证正式验收现要求固定 5,000ms pacing，并在评估结果和保存的 summary
中记录实际配置。其他间隔仍可用于诊断，不能通过正式验收；公共入口场景
保持原有评估规则。这里的 pacing 是每轮请求完成后的等待时间，不是保证
固定 RPS 的开环负载模型。

本机 Node.js 24.11.1 下，修复后的容量门禁测试 25 项、被动趋势测试 14 项
通过。回归覆盖健康的 100 用户 / 30 秒 ramp / 五分钟结果在 5 秒 pacing
下通过、同样输入改为 10 秒后失败，以及实际 runner summary 记录配置值。
后端包 type-check 和脚本/测试文件的单独 TypeScript 检查通过，完整后端
lint 为 0 errors / 1 个既有 `sentry-init.ts` warning，`git diff --check`
通过。独立复核未发现阻塞项，原有目标、会话和遥测门槛未削弱。

没有新增依赖或启动收费资源；这些脚本测试不替代真实登录后的持续负载。
本地 Docker 服务仍停止，数据库和网站测试端口均未监听；上一轮服务器启动
的审批阻塞没有得到解除，本轮没有重试或绕过。改动仅涉及手动验收工具、
...
## 2026-09-10：补测连接池等待持续时间

上一轮记录了队列峰值 2，但没有等待时长。这轮保持 PostgreSQL 17.11、
1 CPU / 256 MiB、三连接池、100 虚拟用户、30 秒 ramp、五分钟稳定阶段和
5,000ms pacing，新增取连接耗时与队列状态持续时间的诊断，保留上一轮
脚本和原始结果。未修改网站运行代码、连接池大小或容量验收阈值。

### 测量方法与校验

本机 `pg-pool` 3.10.1 的 `connect()` 在已有空闲连接时，仍会将回调放入
pending queue，再通过 `process.nextTick` 取出；因此立即读取的队列峰值
不一定表示连接不足。这与 [node-postgres Pool 文档](https://node-postgres.com/apis/pool)
说明的空闲连接分配行为一致。

新诊断保留原来的 `pool.query` 和应用查询观测包装，仅在本地包装
`connect(callback)`，按每次调用记录收到客户端或错误回调的实际耗时。
该耗时包含新连接建立及事件循环调度，并非纯粹的数据库服务端执行时间。
同时在 connect 返回/回调及 acquire、release、remove 事件读取公开池状态，
分别记录队列非空、队列大于 1、三连接名额已满且没有空闲连接时排队的累计
时长和最长连续区间。50ms 定时样本及实际最大采样间隔单独记录，避免
把低频采样误称为连续遥测。区间边界来自 JavaScript 观测，包含观测开销，
不能当作数据库内部队列的精确时间。前两项包含空闲连接分配时的短暂
排队；第三项排除了该情况，但仍可能包含新物理连接建立，不能直接等同
于三条连接均在服务端执行 SQL。

真实持续负载前，另建一个同配置的本地连接池完成两项校验：先占用全部
三条连接，让两次 `SELECT 1` 排队，设置 150ms 延时后释放，再在空闲池执行一次
读取。脚本要求前者出现队列峰值 2、队列大于 1 和全忙排队最长区间均
至少 100ms；后者出现队列峰值 1，但全忙排队峰值为 0。控制池随后关闭，
控制查询不计入持续负载的查询数、延迟和队列统计。

独立脚本的严格 TypeScript 检查通过；错误确认标记仍在建立数据库连接前
被拒绝，测试查询数为 0。短测再次真实执行 1 项并通过，耗时 4,985ms。

### 本轮实测结果

| 指标 | 结果 |
| --- | --- |
| 完成用户 / 稳定阶段时长 | 100 / 300,003.97ms |
| ramp / 稳定阶段成功读取 | 100 / 5,986 |
| 总成功读取 / 失败、超时、连接池错误 | 6,086 / 均为 0 |
| 查询延迟 p50 / p95 / p99 / 最大值 | 2.80 / 9.21 / 21.79 / 112.93ms |
| 取连接耗时 p50 / p95 / p99 / 最大值 | 0.056 / 0.153 / 2.277 / 63.339ms |
| 取连接耗时大于 10ms / 50ms 的次数 | 10 / 2 |
| 连接数峰值 / 队列峰值 | 3 / 1 |
| 队列非空：次数 / 累计 / 最长区间 | 6,078 / 462.521ms / 16.516ms |
| 队列大于 1：次数 / 累计 | 0 / 0ms |
| 连接名额满且无空闲连接时排队：次数 / 累计 / 最长区间 | 117 / 321.531ms / 11.585ms |
| 定时样本：总数 / 队列非空 / 队列大于 1 | 5,495 / 15 / 0 |
| 定时采样实际最大间隔 | 122.456ms |
| 本地数据库诊断 / 退出码 | `passed: true` / 0 |

取连接统计恰好覆盖 6,086 次测量查询，没有漏计；上述查询延迟仍包含
ramp 与稳定阶段全部读取，不含预热和控制查询。观察窗口为 330,020.052ms；
含控制检查等开销的总时间为 330,237.73ms。控制场景实际捕捉到全忙排队
157.110ms、队列大于 1 持续 156.628ms；空闲池仅出现 0.027ms 的非空队列，
全忙排队为 0，支持两类状态的区分。

本次观察到的满额排队区间较短，取连接延迟也较低，没有证据支持为这组
本地 `SELECT 1` 负载增加连接数。上轮峰值 2 本次没有复现，不能反推
上轮峰值的原因或持续时间；原失败报告保留，本轮也没有改善实现后再
作性能对比，因此不能把不同轮次的延迟差异当作优化收益。

本地诊断通过不等于网站发布容量门禁通过：已有认证场景还要求定时样本
中没有排队、连接利用率低于门槛，以及真实应用请求、认证和运行时遥测。
本次 50ms 样本有 15 次队列非空，不能冒充满足该更严格门禁；这些样本
也不是网站门禁的同频率、同路径输入。本轮仍明确标记
`authenticatedUserCapacityClaim: false`、`releaseCapacityClaim: false`。

完整结果为 `.dev-logs/capacity-pg17-queue-sustained-summary.json`，日志为
`.dev-logs/capacity-pg17-queue-sustained.log`，运行配置为
`.dev-logs/capacity-pg17-queue-runtime.json`。新脚本为
`.dev-logs/capacity-pg17-queue-sustained.mts`（SHA-256：
`B0FE54C2E392B823AB448DA7445303E51428D51AE9DF207F924E07487F923F8D`）及
`.dev-logs/capacity-pg17-queue-metrics.mts`（SHA-256：
`380B5CA882EDFE45F7B301525EE56DCAC3B9422C68CBCFFAE59445F815D907DB`）。
启动与清理脚本为 `.dev-logs/capacity-pg17-queue-run.ps1`；以上文件均被
Git 忽略，旧脚本与旧摘要未被覆盖。独立复核确认原查询路径保持不变，
并要求按上述口径区分短暂排队、满额排队与完整取连接耗时。

测试容器已成功停止并自动移除，复核其名称不存在、55432 端口无监听。
本轮未连接生产数据库、增加付费资源或改变生产配置，提交仅包含验证记录。
...
2. 本地 100 个独立登录会话的五分钟 `authenticated_sustained_read_only` 已于
   ...
   检查来压测生产。
   具体改动、实施顺序与复测标准见
   [100 会话后的免费后端优化方案](2026-09-11-100-session-backend-optimization-plan.md)
   （方案尚未实施或部署）。
