...
test("audit-log RLS init-plan batch pins all four ownership policies", () => {
  const manifest = loadApprovedBatchManifest();
  const batch = manifest.batches.find(({ batch_id: batchId }) =>
    batchId === "audit-log-rls-initplan-v1");
  assert.ok(batch);
  assert.equal(batch.source_ref, "33a10e3588de01103d155deb87926b39c3604413");
  assert.equal(batch.mode, "transactional");
  ...
  assert.deepEqual(batch.migrations[0], {
    ...
    sha256: "1366ba5d630fe3eff786920ae10bce19315af13891eecb20d62603934384cb02",
  });
  ...
  for (const phase of [batch.preconditions, batch.postconditions]) {
    ...
    assert.equal(assertions.filter(({ kind }) => kind === "rls_enabled").length, 4);
  }
