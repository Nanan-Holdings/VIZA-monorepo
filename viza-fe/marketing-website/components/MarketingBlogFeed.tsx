          ...
          <div className="flex flex-1 flex-col p-6">
            <div className="mb-4 flex flex-wrap items-center gap-x-3 gap-y-2 text-xs text-fg-2">
              {post.category && categorySlug(post.category) ? (
                <Link href={`/blog/category/${categorySlug(post.category)}`} locale={locale} className="rounded-pill bg-brand-50 px-3 py-1 font-medium text-brand-600 hover:bg-brand-100">
                  {post.category}
                </Link>
              ) : null}
