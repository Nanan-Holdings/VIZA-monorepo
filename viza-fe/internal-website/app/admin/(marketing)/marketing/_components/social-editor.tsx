...
import { VIZA_CONTENT_PLATFORMS, type MarketingBlogAdminRecord, type MarketingSocialCompositionRecord, type MarketingSocialPlatform } from "@/lib/marketing/contracts";
import type { InterfaceLocale } from "@/lib/i18n/locale";
import { MarketingAssetUpload } from "./asset-upload";

const PLATFORM_LABELS: Record<MarketingSocialPlatform, string> = { x: "X", "google-business-sg": "Google Business SG", instagram: "Instagram", linkedin: "LinkedIn", facebook: "Facebook", pinterest: "Pinterest", reddit: "Reddit" };
const COPY = {
...
export function SocialEditor({ locale, connectedPlatforms, openrouterConnected, canPublish, initial, sourceBlog, sourceBlogUrl }: { locale: InterfaceLocale; connectedPlatforms: MarketingSocialPlatform[]; openrouterConnected: boolean; canPublish: boolean; initial?: MarketingSocialCompositionRecord; sourceBlog?: MarketingBlogAdminRecord; sourceBlogUrl?: string }) {
  ...
  return (
    <Card><CardHeader><CardTitle>{copy.title}</CardTitle><CardDescription>{copy.help}</CardDescription></CardHeader><CardContent><form action={(data) => submit(data, false)} className="grid gap-4 md:grid-cols-2">
      ...
      <Field label={copy.destination}><Input name="destinationUrl" type="url" defaultValue={initial?.destinationUrl ?? sourceBlogUrl ?? ""} /><label className="mt-2 flex items-center gap-2 text-xs text-muted-foreground"><Checkbox checked={trackDestination} onCheckedChange={(checked) => setTrackDestination(checked === true)} />{copy.track}</label></Field><Field label={copy.media}><Input name="mediaUrl" type="url" value={mediaUrl} onChange={(event) => setMediaUrl(event.target.value)} /><MarketingAssetUpload kind="image" locale={locale} onUploaded={setMediaUrl} /></Field><Field label={copy.document}><Input name="documentUrl" type="url" value={documentUrl} onChange={(event) => setDocumentUrl(event.target.value)} /><MarketingAssetUpload kind="document" locale={locale} onUploaded={setDocumentUrl} /></Field>{supportsScheduling ? <Field label={copy.schedule}><Input name="scheduledFor" type="datetime-local" defaultValue={initial?.scheduledFor?.slice(0, 16)} /></Field> : <p className="self-center text-sm text-muted-foreground">{locale === "zh" ? "Instagram 和 Pinterest 内容可在审核后即时发布。" : "Instagram and Pinterest posts publish when approved."}</p>}
      <div className="space-y-3 md:col-span-2"><Label>{copy.content}</Label><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{VIZA_CONTENT_PLATFORMS.map((platform) => { const selected = platforms.includes(platform); const connected = connectedPlatforms.includes(platform); return <div key={platform} className="rounded-lg border p-3"><label className="flex items-center gap-2 text-sm font-medium"><Checkbox checked={selected} onCheckedChange={(checked) => setPlatforms((current) => checked ? [...current, platform] : current.filter((item) => item !== platform))} />{PLATFORM_LABELS[platform]}<span className={connected ? "ml-auto text-xs text-emerald-700" : "ml-auto text-xs text-muted-foreground"}>{connected ? "●" : "○"}</span></label>{selected ? <Textarea className="mt-3" name={`content-${platform}`} rows={5} required defaultValue={initial?.platformContent[platform] ?? ""} /> : null}</div>; })}</div></div>
      ...
      <div className="flex flex-wrap gap-2 md:col-span-2"><Button disabled={pending || !platforms.length} type="submit">{pending ? copy.saving : copy.save}</Button><Button disabled={pending || !platforms.length || !openrouterConnected} type="button" variant="secondary" onClick={(event) => { const form = event.currentTarget.form; if (form?.reportValidity()) generate(new FormData(form)); }}>{copy.generate}</Button>{canPublish ? <Button disabled={pending || !platforms.length || !publishAvailable} type="button" variant="outline" onClick={(event) => { const form = event.currentTarget.form; if (form?.reportValidity()) submit(new FormData(form), true); }}>{copy.savePublish}</Button> : null}</div>
