...
export default async function AppointmentOperationsPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-7xl space-y-6 p-4 md:p-8">
      ...
      {errors.length > 0 ? <Alert variant="warning"><AlertIcon variant="warning" /><AlertTitle>{copy.unavailable}</AlertTitle><AlertDescription>{errors.map((error) => <p key={error} className="font-mono">{error}</p>)}</AlertDescription></Alert> : null}
