...
export function PrivacyExecutionControls({
  ...
  return (
    <div className="mt-4 rounded-lg border border-[#d9dee7] bg-[#fafbfc] p-4">
      ...
      {message ? <Alert className="mt-2" variant={message === "✓" ? "success" : "destructive"}><AlertIcon variant={message === "✓" ? "success" : "destructive"} /><AlertDescription>{message}</AlertDescription></Alert> : null}
    </div>
  );
}
