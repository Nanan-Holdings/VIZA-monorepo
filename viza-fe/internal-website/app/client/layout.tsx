...
function ClientLayoutContent({
  ...
  useEffect(() => {
    ...
    document.body.classList.add("client-icon-system");

    return () => {
      document.body.style.backgroundColor = previousBodyBackground;
      document.body.classList.remove("client-icon-system");
    };
  }, [isReportPage]);
  ...
  if (isAuthPage) {
    return (
      <div className="client-icon-system min-h-screen bg-white font-sans">
        <main>{children}</main>
      </div>
    );
  }
  ...
  if (sessionValid === "invalidated") {
    return (
      <div className="client-icon-system client-shell min-h-screen bg-[#fafafa] font-sans flex items-center justify-center">
        <div className="flex flex-col items-center gap-6 max-w-md text-center px-4">
          <div className="w-16 h-16 rounded-full bg-amber-100 flex items-center justify-center">
            <Warning className="size-8 text-amber-600" weight="regular" />
          </div>
          <div>
  ...
  return (
    <div
      className={clsx(
        "client-icon-system client-shell min-h-screen font-sans",
