...
interface SubscriptionQueryBuilder<Row> extends PromiseLike<QueryResponse<Row[]>> {
  eq(column: string, value: unknown): SubscriptionQueryBuilder<Row>;
  insert(values: unknown): SubscriptionQueryBuilder<Row>;
  maybeSingle(): Promise<QueryResponse<Row>>;
  select(columns?: string): SubscriptionQueryBuilder<Row>;
  single(): Promise<QueryResponse<Row>>;
  update(values: unknown): SubscriptionQueryBuilder<Row>;
}

interface SubscriptionAdminClient {
  from(table: "payment_records"): SubscriptionQueryBuilder<PaymentRecordRow>;
}
...
export function createSubscriptionAdminClient(): SubscriptionAdminClient {
  return createAdminClient() as unknown as SubscriptionAdminClient;
}
