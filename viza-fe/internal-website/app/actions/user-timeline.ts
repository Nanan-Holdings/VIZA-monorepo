...
export async function getUserTimelineStatus(): Promise<{
  ...
  try {
    ...
    const [
      ...
      // 1. ShenAI profile_data: check for shenai metric in profile_data table (stub)
      (adminClient as any)
        .from("profile_data")
      ...
      // 2. Lab orders: count actual bookings (with appointment dates) for first/second
      (adminClient as any)
        .from("lab_orders")
      ...
      // 3. Lab results: fetch completed lab orders (for count + date fallback)
      (adminClient as any)
        .from("lab_orders")
      ...
      // 4. Action plans: check if any plan exists
      (adminClient as any)
        .from("action_plans")
    ...
    // 1. ShenAI (stub — always pending until integration lands)
    const shenaiCompleted = (shenaiResult.count ?? 0) > 0;

    // 2. First blood panel: any booked lab_order
    const labOrders = labOrdersResult.data ?? [];

    // 3. Lab results: any completed lab order (also used to infer blood panel was done)
    ...
    // 4. Action plan: any action_plans record
