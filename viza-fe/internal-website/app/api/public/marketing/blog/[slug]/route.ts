...
export async function GET(request: Request, context: { params: Promise<{ slug: string }> }) {
  ...
  const { data, error } = await db.from("marketing_blog_posts").select("id, locale, slug, title, excerpt, body_markdown, cover_image_url, category, author_name, seo_title, seo_description, metadata, published_at, updated_at").eq("locale", locale).eq("slug", slug).eq("status", "published").maybeSingle();
  if (error) {
