...
--
-- The API normally uses the service_role after checking application ownership,
-- but these tables are exposed to authenticated clients as a defense in depth.
-- RLS therefore repeats the application -> applicant profile ownership check.
...
CREATE TABLE IF NOT EXISTS public.form_assistant_messages (
  ...
  -- These denormalized columns make application-scoped retention and indexes
  -- cheap. They are nullable for compatibility with session-only service-role
  -- inserts; RLS always derives ownership from session_id.
