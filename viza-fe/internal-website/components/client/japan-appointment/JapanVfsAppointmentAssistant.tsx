...
export function JapanVfsAppointmentAssistant({ applicationId }: Props) {
  ...
  const protectedStagesEnabled = job?.userPreferencesJson.automationMode === "assisted_live_with_account";
  ...
  return (
    <main className="mx-auto w-full max-w-4xl space-y-6 py-8">
      ...
      {!activeJob && <><Card><CardHeader><CardTitle>{t("eligibility.title")}</CardTitle></CardHeader><CardContent className="grid gap-4 sm:grid-cols-2">
      ...
      <Card><CardHeader><CardTitle>{t("workflow.title")}</CardTitle></CardHeader><CardContent className="space-y-4">
        {!activeJob && <label className="flex min-h-11 items-start gap-3 rounded-lg border p-3 text-sm"><Checkbox checked={consent} onCheckedChange={(value) => setConsent(value === true)} /><span>{t("workflow.consent")}</span></label>}
        {!activeJob && consentRecorded && <Alert className="border-emerald-200 bg-emerald-50"><CheckCircle2 className="h-4 w-4 text-emerald-700" /><AlertTitle>{t("workflow.consentRecorded")}</AlertTitle><AlertDescription>{t("workflow.consentRecordedBody")}</AlertDescription></Alert>}
        ...
        {!activeJob ? <div className="grid gap-3 sm:grid-cols-2">
        ...
        </div> : <Alert className="border-brand-100 bg-brand-50"><ShieldCheck className="h-4 w-4 text-brand-600" /><AlertTitle>{t("workflow.publicOnlyTitle")}</AlertTitle><AlertDescription>{t("workflow.publicOnlyBody")}</AlertDescription></Alert>}
        ...
        {activeJob && <BrandActionButton className="w-full" variant="secondary" loading={busy === "check"} loadingText={t("workflow.checking")} disabled={busy !== null} onClick={() => action("check", () => checkJapanAppointmentPortal(activeJob.id))}><RefreshCw />{t("workflow.check")}</BrandActionButton>}
      ...
      {job && protectedStagesEnabled && <Card><CardHeader><CardTitle>{t("slots.title")}</CardTitle></CardHeader><CardContent className="space-y-3">
      ...
      {job && protectedStagesEnabled && <Card><CardHeader><CardTitle>{t("payment.title")}</CardTitle></CardHeader><CardContent className="space-y-4">
      ...
      {job && protectedStagesEnabled && <Card><CardHeader><CardTitle>{t("final.title")}</CardTitle></CardHeader><CardContent className="space-y-4">
      ...
      <Card><CardHeader><CardTitle>{t("evidence.title")}</CardTitle></CardHeader><CardContent className="grid gap-3 sm:grid-cols-2"><Detail label={t("evidence.httpStatus")} value={String(snapshot?.evidence?.httpStatus ?? "-")} /><Detail label={t("evidence.pageTitle")} value={String(snapshot?.evidence?.pageTitle ?? "-")} /><Detail label={t("evidence.observedAt")} value={String(snapshot?.evidence?.observedAt ?? "-")} /><Detail label={t("evidence.entryClick")} value={snapshot?.evidence?.publicEntryClicked ? t("yes") : t("no")} /><Detail label={t("evidence.replay")} value={snapshot?.evidence?.browserbaseReplayAvailable ? t("yes") : t("no")} /></CardContent></Card>
    </main>
  );
}
