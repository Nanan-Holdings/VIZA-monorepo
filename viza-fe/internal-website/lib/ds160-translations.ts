...
const ZH_LABELS: Record<string, string> = {
  ...
  "Have you been ten-printed?": "您是否曾采集过十指指纹？",
...
/**
 * Chinese labels for the complete region list exposed by the DS-160 US
 * state/territory selector.  The selector stores the two-letter CEAC value;
 * these labels are display-only and must never replace that canonical value.
 */
export const DS160_US_REGION_ZH_BY_CODE: Readonly<Record<string, string>> = {
  AL: "阿拉巴马州",
  AK: "阿拉斯加州",
  AS: "美属萨摩亚",
  AZ: "亚利桑那州",
  ...
  WY: "怀俄明州",
  AA: "驻美洲武装部队",
  AE: "驻欧洲、加拿大、非洲和中东武装部队",
  AP: "驻太平洋武装部队",
};

const DS160_US_REGION_ZH_BY_NAME: Readonly<Record<string, string>> = {
  Alabama: "阿拉巴马州",
  Alaska: "阿拉斯加州",
  ...
  Wisconsin: "威斯康星州",
  Wyoming: "怀俄明州",
  "Armed Forces Americas": "驻美洲武装部队",
  "Armed Forces Europe, Canada, Africa and Middle East": "驻欧洲、加拿大、非洲和中东武装部队",
  "Armed Forces Pacific": "驻太平洋武装部队",
};

const DS160_US_REGION_ZH_BY_NORMALIZED_NAME = Object.entries(DS160_US_REGION_ZH_BY_NAME).reduce<Record<string, string>>(
  (labels, [name, label]) => {
    labels[name.toLowerCase()] = label;
    return labels;
  },
  {},
);

/** Resolve a US region's Chinese display label without changing its code. */
export function translateUsRegionName(name: string, shortCode?: string): string {
  const codeLabel = shortCode ? DS160_US_REGION_ZH_BY_CODE[shortCode.trim().toUpperCase()] : undefined;
  if (codeLabel) return codeLabel;

  return DS160_US_REGION_ZH_BY_NORMALIZED_NAME[name.trim().toLowerCase()] ?? name;
}
...
const ZH_OPTIONS: Record<string, string> = {
  ...
  // Schengen and other DB-driven form options
  "Tourism": "旅游",
  "Business": "商务",
  "Visiting family or friends": "探亲访友",
  "Family or friends": "亲友",
  "Cultural": "文化",
  "Sports": "体育",
  "Official visit": "公务访问",
  "Medical reasons": "医疗原因",
  "Study": "学习",
  ...
  "Special passport": "特殊护照",
  "Other travel document": "其他旅行证件",
  "Passport": "护照",
  "National identity card": "国民身份证",
  "Registered partnership": "注册伴侣关系",
  "Registered partner": "注册伴侣",
  "Widow(er)": "丧偶",
  "Grandchild": "孙子女/外孙子女",
  "Dependent ascendant (parent, grandparent)": "受抚养的直系尊亲属（父母、祖父母/外祖父母）",
  "Other dependent relative": "其他受抚养亲属",
  "EU/EEA/CH citizen": "欧盟/欧洲经济区/瑞士公民",
  "Permanent resident / long-term resident": "永久居民/长期居民",
  "Temporary resident / residence permit holder": "临时居民/居留许可持有人",
  "Hotel or other commercial accommodation": "酒店或其他商业住宿",
  "With inviting person (already declared in host details)": "与邀请人同住（已在邀请人信息中声明）",
  "Rented accommodation (short-let / Airbnb / apartment)": "租赁住宿（短租 / Airbnb / 公寓）",
  "Myself": "本人",
  "A sponsor (host, company, organisation, other)": "担保人（接待方、公司、机构或其他）",
  "Both myself and a sponsor": "本人和担保人共同承担",
  "Cash": "现金",
  "Traveller's cheques": "旅行支票",
  "Credit card": "信用卡",
  "Pre-paid accommodation": "已预付住宿",
  "Pre-paid transport": "已预付交通",
  "Other means": "其他方式",
  "Others": "其他",
  "Single-entry": "单次入境",
  "Multiple-entry": "多次入境",
  ...
  "Sikhism": "锡克教",
  "Jainism": "耆那教",
  "Zoroastrianism (Parsi)": "琐罗亚斯德教（帕西）",
  "Taoism": "道教",
  "No religion": "无宗教信仰",
  "Self-employed": "自雇",
  "Self-Employed": "自雇",
  ...
  "Resort": "度假村",
  "Guesthouse": "旅馆",
  "Guesthouse / Homestay": "旅馆/民宿",
  "Guesthouse / Hostel": "旅馆/青旅",
  "Hostel / Guesthouse": "青旅/旅馆",
  "Guesthouse / Bed & breakfast": "旅馆/民宿（含早餐）",
  "Hostel / Bed & breakfast": "青旅/民宿（含早餐）",
  "Hostel / Backpacker / Bed & breakfast": "青旅/背包客旅馆/民宿（含早餐）",
  "Motel": "汽车旅馆",
  "Villa / Short-term rental": "别墅/短租",
  "Resort / All-inclusive": "度假村/全包式住宿",
  "Resort / Integrated Resort": "度假村/综合度假村",
  "Safari lodge": "野奢营地",
  "Ashram / Spiritual retreat": "修行所/灵修静修处",
  "I agree": "我同意",
  "I don't know": "我不知道",
  "I'm not sure": "我不确定",
  "Unsure": "不确定",
  "Prefer not to say / Unspecified": "不愿透露/未说明",
  "Indeterminate / Intersex / Unspecified": "未定/双性/未说明",
  "Transgender": "跨性别",
  "Another gender": "其他性别",
  "De facto": "事实伴侣关系",
  "Common-law": "同居伴侣关系",
  ...
  "Annulled marriage": "婚姻无效",
  "Married / Civil union": "已婚/民事结合",
  "Maiden name": "婚前姓",
  "Professional name": "职业名",
  "Religious name": "宗教名",
  "Anglicised name": "英语化姓名",
  "Ceased / renounced": "已终止/已放弃",
  "Aunt / Uncle": "姨/姑/舅/叔/伯",
  "Cousin": "堂表亲",
  "In-law": "姻亲",
  "Visitor": "访客",
  "Tourist": "游客",
  "Work": "工作",
  "Work Visa": "工作签证",
  "Holiday / sightseeing": "度假/观光",
  "Social or recreational activity": "社交或休闲活动",
  "Attend a meeting or conference": "参加会议或大会",
  "Meeting / Conference": "会议/大会",
  "Conference (attending an MEA-cleared conference)": "会议（参加印度外交部批准的会议）",
  "Trade fair or exhibition": "贸易展会或展览",
  "Trade / Investment / Intra-Company Transfer": "贸易/投资/公司内部调动",
  "Media / Journalist": "媒体/记者",
  "Clinical attachment": "临床见习",
  "Dental observer post": "牙科观察员岗位",
  "PLAB test": "PLAB 考试",
  "OSCE test": "OSCE 考试",
  "Civil Servant": "公务员",
  "Entrepreneur": "创业者",
  "Homemaker / Caregiver": "家庭照护者",
  "Housewife / Homemaker": "家庭主妇/家庭照护者",
  "Marriage Migrant": "婚姻移民",
  "Inviter in Japan": "在日邀请人",
  "Self-funded": "自费",
  "Tour operator (ADS stream)": "旅行社（ADS 类别）",
  "Scholarship or grant": "奖学金或补助金",
  "Scholarship / grant": "奖学金/补助金",
  "I am paying": "我本人支付",
  "Someone else is paying for me": "他人为我支付",
  "Own property": "自有房产",
  "I own it": "我拥有该住所",
  "I rent it": "我租住该住所",
  "Settled (Indefinite Leave to Remain)": "定居身份（无限期居留）",
  "Yes, TB test required": "是，需要结核病检测",
  "No, not required": "否，不需要",
  "English": "英语",
  "Chinese": "华裔",
  "Malay": "马来人",
  "Indian": "印度人",
  "Eurasian": "欧亚混血",
  "Caucasian": "白人",
  "Authorised Visa Agent": "授权签证代理",
  "Short-term (≤ 90 days)": "短期（不超过90天）",
  "Long-term (> 90 days)": "长期（超过90天）",
  "Master's / Doctoral": "硕士/博士",
  "Bachelor's": "学士",
  "Humanitarian (cultural, sporting, scientific events)": "人道主义（文化、体育、科学活动）",
  "Campervan / Self-contained RV": "露营车/自备设施房车",
  "Liveaboard / Safari boat": "船宿/观光船",
  "Yacht": "游艇",
...
const GENERIC_ZH_TEXT: Record<string, string> = {
  ...
  "Supporting Documents": "辅助材料",
  "UKVI account password": "UKVI 账号密码",
  "UKVI Resume URL": "UKVI 续填申请链接",
  ...
  "Is the application being filled in by someone other than the applicant?": "本申请表是否由申请人以外的其他人填写？",
  "Surname of the person filling in the application form": "填写本申请表人员的姓氏",
  "First name(s) of the person filling in the application form": "填写本申请表人员的名字",
  "Address of the person filling in the application form": "填写本申请表人员的地址",
  "E-mail address of the person filling in the application form": "填写本申请表人员的电子邮箱",
  "Telephone number of the person filling in the application form": "填写本申请表人员的电话号码",
  "Place of application": "申请提交地点",
  "Date of signing": "签署日期",
  ...
  "I am aware that the visa fee is not refunded if the visa is refused.": "我知悉，如签证被拒，签证费不予退还。",
  "Applicable if a multiple-entry visa is issued: I am aware of the need to have adequate travel medical insurance for my first stay and any subsequent visits to the territory of Member States.": "如获发多次入境签证：我知悉首次停留以及之后每次进入成员国领土时，均须持有足额旅行医疗保险。",
  "I am aware of and consent to the following: the collection of the data required by this application form and the taking of my photograph and, if applicable, the taking of fingerprints, are mandatory for the examination of the application; and any personal data concerning me which appear on the application form, as well as my fingerprints and my photograph, will be supplied to the relevant authorities of the Member States and processed by those authorities, for the purposes of a decision on my application. Such data will be entered into and stored in the Visa Information System (VIS) for a maximum period of five years.": "我知悉并同意：审理本申请必须收集本申请表所需数据、采集本人照片，并在适用时采集指纹；申请表中的个人数据以及本人的指纹和照片，将提供给成员国相关主管机关并由其处理，用于就本申请作出决定。上述数据将录入并存储于签证信息系统（VIS），最长保存五年。",
  "I am aware that I have the right to obtain, in any of the Member States, notification of the data relating to me recorded in the VIS and of the Member State which transmitted the data, and to request that data relating to me which are inaccurate be corrected and that data relating to me processed unlawfully be deleted.": "我知悉，本人有权在任何成员国获知 VIS 中记录的本人相关数据以及传输该数据的成员国；并有权要求更正不准确的本人数据，或删除被非法处理的本人数据。",
  "I declare that to the best of my knowledge all particulars supplied by me are correct and complete.": "我声明，据本人所知，本人提供的所有信息均正确且完整。",
  "I am aware that any false statement will lead to my application being rejected or to the annulment of a visa already granted and may render me liable to prosecution under the law of the Member State which deals with the application.": "我知悉，任何虚假陈述都将导致本申请被拒，或已获签证被撤销，并可能使本人依处理本申请成员国的法律承担被追诉责任。",
  "I undertake to leave the territory of the Member States before the expiry of the visa, if granted. I have been informed that possession of a visa is only one of the prerequisites for entry into the European territory of the Member States.": "如获发签证，我承诺在签证到期前离开成员国领土。我已被告知，持有签证只是进入成员国欧洲领土的前提条件之一。",
  "Is there anything else you would like to tell us about your application?": "关于本申请，您还有其他需要说明的信息吗？",
...
const FIELD_NAME_ZH: Record<string, string> = {
  ...
  ukvi_resume_url: "UKVI 续填申请链接",
  ukvi_account_email: "UKVI 账号邮箱",
  ukvi_account_username: "UKVI 账号用户名",
  has_different_filler: "是否由他人填写申请表",
  filler_surname: "填写人姓氏",
  filler_given_names: "填写人名字",
  filler_address: "填写人地址",
  filler_email: "填写人电子邮箱",
  filler_phone: "填写人电话号码",
  place_of_application: "申请提交地点",
  declaration_date: "签署日期",
  declaration_fee_not_refunded_awareness: "签证费不退还知悉声明",
  declaration_insurance_multi_entry_awareness: "多次入境旅行医疗保险知悉声明",
  declaration_vis_consent: "VIS 数据采集与处理同意声明",
  declaration_data_rights_awareness: "VIS 数据权利知悉声明",
  declaration_truthfulness: "信息真实完整声明",
  declaration_awareness_refusal: "虚假陈述后果知悉声明",
  declaration_undertaking_to_leave: "签证到期前离境承诺",
...
export function getEnglishPlaceholder(placeholder: string | null): string | null {
  ...
  if (hasChinese(placeholder)) {
    if (/选择国家/.test(placeholder)) return "Select country...";
    if (/请选择/.test(placeholder)) return "Select...";
  }
