...
export function marketingCategorySlug(name: string): string {
  return name.normalize("NFKD").replace(/[\u0300-\u036f]/g, "").toLowerCase().replace(/[^\p{L}\p{N}]+/gu, "-").replace(/^-|-$/g, "");
}

export async function revalidatePublicMarketingBlog(input: { locale: MarketingBlogLocale; slug: string; category?: string }) {
