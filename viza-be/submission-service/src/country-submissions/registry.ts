...
const JP_VJW_REQUIRED_FIELDS: FieldRequirement[] = [
  ...
  arrivalCardField("residence_city", "City of residence", "personal"),
  arrivalCardField("occupation", "Occupation", "personal"),
  arrivalCardField("arrival_date", "Arrival date", "trip"),
  arrivalCardField("arrival_airport", "Intended port of entry", "trip"),
  arrivalCardField("arrival_airline", "Arrival airline", "trip"),
  ...
  arrivalCardField("has_prohibited_goods", "Prohibited goods declaration", "security"),
  arrivalCardField("has_restricted_goods", "Restricted goods declaration", "security"),
  arrivalCardField("has_gold_or_gold_products", "Gold/gold products declaration", "security"),
...
const CONFIGS: ProviderConfig[] = [
  ...
  },
  {
    ...
    displayName: "Taiwan Online Entry Permit",
    ...
    routeStatus: "runner_job_dispatched",
    serviceFiles: ["src/tw/**"],
    schemaFiles: ["../agent-backend/scripts/seed-tw-entry-permit-form-fields.ts"],
    mapperFiles: ["src/country-submissions/from-records.ts", "src/tw/normalize.ts"],
    automationFiles: ["src/tw/apply.ts", "src/tw/runner.ts"],
    notes: "Single continuous session (no persistent portal account): terms modal, delivery-location tab, application-form tab with inline email OTP verification, every field/file, shared 2captcha final CAPTCHA solve, official \"確認資料\" submit, then fail-closed official receipt capture. No payment in this session — NIA approval + payment happen later on the official site.",
  },
  {
    countryCode: "ID",
  ...
  },
  {
    ...
    automationFiles: ["src/jp-vjw/runner.ts", "src/jp-vjw/live-adapter.ts"],
    requiredFields: JP_VJW_REQUIRED_FIELDS,
    includeAllAnswersInPayload: true,
    dryRunConfirmationPrefix: "DRYRUN-JP-VJW",
    notes: "Dedicated Visit Japan Web arrival/customs declaration. Live success requires a persisted QR element from the official vjw.digital.go.jp QR route; realSubmitAvailable remains false until a controlled end-to-end live smoke succeeds, and the existing JP_TOURIST paper form remains separate.",
  },
  {
  ...
  },
  {
    ...
    automationFiles: ["src/ph-etravel/runner-job.ts", "src/ph-etravel/runner.ts", "scripts/run-ph-etravel-smoke.ts"],
    requiredFields: PH_ETRAVEL_REQUIRED_FIELDS,
    includeAllAnswersInPayload: true,
    dryRunConfirmationPrefix: "DRYRUN-PHETRAVEL",
    notes: "Canonical runner_job performs local 72-hour, duplicate/recovery, and launch-preflight gates before any account or browser work. Live assisted arrival remains fail-closed at Review; final Submit, authoritative post-submit registration read, reference-derived QR validation, and atomic result/queue synchronization remain blocking release evidence.",
  },
  {
    countryCode: "PH",
    countryAliases: ["ph", "philippines"],
