...
async function fillCardForm(page: Page, applicantId: string): Promise<void> {
  ...
  const fillIfExists = async (selector: string, value: string) => {
    try {
      ...
      // selector miss — caller's fallback handles it
    }
  };
...
export async function payLaWithEscrowCard(input: PayLaInput): Promise<PayLaResult> {
  ...
  try {
    ...
    if (!submitted) {
      return { status: "needs_human", portalReceiptId: null, reason: "could not locate LA payment submit button" };
    }
    ...
    if (/declined|insufficient funds|payment failed/i.test(bodyText)) {
      await recordPortalDecline(orderId, "card declined on LA portal");
      return { status: "declined", portalReceiptId: null, reason: "LA portal returned decline" };
    }
    const receiptId = extractReceiptFromText(bodyText);
    if (!receiptId) {
      return { status: "needs_human", portalReceiptId: null, reason: "LA portal did not surface a receipt id post-submit" };
    }
    ...
    return { status: "needs_human", portalReceiptId: null, reason: `LA pay threw: ${msg}` };
  }
}
