...
export async function payLkWithEscrowCard(input: PayLkInput): Promise<PayLkResult> {
  ...
  try {
    ...
    if (!submitted) {
      return { status: "needs_human", portalReceiptId: null, reason: "could not locate LK payment submit button" };
    }
    ...
    if (/declined|insufficient funds|payment failed/i.test(bodyText)) {
      await recordPortalDecline(orderId, "card declined on LK portal");
      return { status: "declined", portalReceiptId: null, reason: "LK portal returned decline" };
    }
    const receiptId = extractReceiptFromText(bodyText);
    if (!receiptId) {
      return { status: "needs_human", portalReceiptId: null, reason: "LK portal did not surface a receipt id post-submit" };
    }
    ...
    return { status: "needs_human", portalReceiptId: null, reason: `LK pay threw: ${msg}` };
  }
}
