...
export async function payZaWithEscrowCard(input: PayZaInput): Promise<PayZaResult> {
  ...
  try {
    ...
    if (!submitted) {
      return { status: "needs_human", portalReceiptId: null, reason: "could not locate ZA payment submit button" };
    }
    ...
    if (/declined|insufficient funds|payment failed/i.test(bodyText)) {
      await recordPortalDecline(orderId, "card declined on ZA portal");
      return { status: "declined", portalReceiptId: null, reason: "ZA portal returned decline" };
    }
    const receiptId = extractReceiptFromText(bodyText);
    if (!receiptId) {
      return { status: "needs_human", portalReceiptId: null, reason: "ZA portal did not surface a receipt id post-submit" };
    }
    ...
    return { status: "needs_human", portalReceiptId: null, reason: `ZA pay threw: ${msg}` };
  }
}
