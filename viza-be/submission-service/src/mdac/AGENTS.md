...
- Shared-pool runs receive a `RunnerExecutionContext`; assert ownership immediately before the final MDAC submit click and close the browser session when its signal aborts. Let ownership-loss/abort errors escape without converting them into a failed portal result.
