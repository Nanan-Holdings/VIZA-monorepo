...
test("normalizes Indonesia C1 tourist answers to the official eVisa portal", () => {
  ...
  assert.equal(normalized.phoneCountryCode, "+44");
  assert.equal(normalized.mobileNumber, "7700900123");
...
test("classifies Indonesia portal login and registration gates", () => {
  ...
  assert.equal(
    classifyIndonesiaPortalSnapshot({
      ...
      text: "The request could not be satisfied. Generated by cloudfront.",
    }),
    "portal_blocked",
    "a Finpay CDN/WAF error must not be reported as a live payment checkpoint",
  );
});
