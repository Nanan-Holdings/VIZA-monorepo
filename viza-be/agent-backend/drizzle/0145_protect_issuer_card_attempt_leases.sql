-- Prevent a second worker from stealing a live issuer-card lease. Recovery by
-- the same worker and takeover after lease expiry remain idempotent.
...
BEGIN
  ...
  END IF;
  IF v_attempt.id IS NOT NULL
     AND v_attempt.locked_by IS NOT NULL
     AND v_attempt.locked_by <> p_worker_id
     AND v_attempt.lease_expires_at > now() THEN
    RAISE EXCEPTION 'issuer-card attempt is leased by another worker until %',
...
COMMENT ON FUNCTION public.claim_issuer_card_attempt(
  UUID, UUID, UUID, TEXT, TEXT, INTEGER
) IS 'Claims one exact allocation/intent attempt without stealing an unexpired lease from another worker.';
