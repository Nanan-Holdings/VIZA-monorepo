...
-- account-level consent audit table. This mirrors the frontend Supabase
-- migration so local and production database histories stay aligned.
...
JOIN applicant_profiles AS profile
  ON profile.id = accepted.applicant_id
WHERE NOT EXISTS (
  SELECT 1
  FROM consent_event AS existing
