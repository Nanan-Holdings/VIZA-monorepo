CREATE TABLE IF NOT EXISTS universal_profile_answers (
...
ALTER TABLE universal_profile_answers ENABLE ROW LEVEL SECURITY;

CREATE UNIQUE INDEX IF NOT EXISTS universal_profile_answers_user_key_idx
  ON universal_profile_answers(auth_user_id, canonical_key);

CREATE INDEX IF NOT EXISTS universal_profile_answers_applicant_idx
  ON universal_profile_answers(applicant_id);

CREATE INDEX IF NOT EXISTS universal_profile_answers_source_app_idx
  ON universal_profile_answers(source_application_id);

REVOKE ALL ON TABLE universal_profile_answers FROM anon, authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE universal_profile_answers TO service_role;

COMMENT ON TABLE universal_profile_answers IS
