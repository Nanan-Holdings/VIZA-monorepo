-- Mirrors the website Supabase migration for Vietnam e-Visa status tracking
-- and result delivery. Existing applications are intentionally not backfilled.
...
CREATE TABLE IF NOT EXISTS official_application_tracking (
  ...
  CONSTRAINT official_application_tracking_country_check CHECK (country_code = 'VN'),
  CONSTRAINT official_application_tracking_status_check CHECK (tracking_status IN ('active', 'completed', 'disabled')),
  CONSTRAINT official_application_tracking_hour_check CHECK (daily_check_hour BETWEEN 0 AND 23),
  CONSTRAINT official_application_tracking_minute_check CHECK (daily_check_minute BETWEEN 0 AND 59)
);
