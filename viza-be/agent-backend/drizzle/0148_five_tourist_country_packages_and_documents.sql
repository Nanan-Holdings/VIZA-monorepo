...
WITH requirements(country, visa_type, requirement_key, label_en, label_zh, description, required, sort_order, metadata) AS (
  VALUES
    ...
    ('united_arab_emirates', 'AE_TOURIST_VISA', 'six_month_bank_statement', 'Six-month bank statement', '六个月银行对账单', 'Official, stamped, signed, colored bank statement covering six months and showing at least USD 4,000 or equivalent in each month.', true, 30, '{"document_type":"bank_statement","accept":[".pdf"],"statement_months":6,"minimum_monthly_balance_usd_equivalent":4000,"must_be_official":true,"must_be_stamped":true,"must_be_signed":true,"must_be_colored":true,"content_review_required":true,"source":"uae_icp_service_783"}'::jsonb),
    ('united_arab_emirates', 'AE_TOURIST_VISA', 'uae_health_coverage_evidence', 'UAE medical insurance', '阿联酋医疗保险', 'Medical-insurance evidence issued in the United Arab Emirates and valid for at least 180 days.', true, 40, '{"document_type":"travel_insurance","accept":[".pdf"],"issuer_country":"United Arab Emirates","minimum_validity_days":180,"content_review_required":true,"source":"uae_icp_service_783"}'::jsonb),
    ...
    ('united_arab_emirates', 'AE_TOURIST_VISA', 'uae_accommodation_evidence', 'UAE accommodation proof', '阿联酋住宿证明', 'Not listed as an always-required public service-card document. Collect only if the authenticated transaction-783 form requests it for the entered data.', false, 60, '{"document_type":"hotel_booking","accept":[".pdf",".jpg",".jpeg",".png"],"source":"uae_icp_service_783","applicability":"conditional","condition_source":"authenticated_transaction_783_request"}'::jsonb),
    ('united_arab_emirates', 'AE_TOURIST_VISA', 'national_identity_copy', 'National identity document', '本国身份证件', 'Optional on the public service card. Collect only if the authenticated transaction-783 form requests it for the entered data.', false, 70, '{"document_type":"national_identity_card","accept":[".pdf",".jpg",".jpeg",".png"],"source":"uae_icp_service_783","applicability":"optional","condition_source":"authenticated_transaction_783_request"}'::jsonb)
), packages AS (
