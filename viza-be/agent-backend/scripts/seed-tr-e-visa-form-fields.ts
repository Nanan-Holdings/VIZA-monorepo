/**
 * Seed script: visa_form_fields for Türkiye (Turkey) e-Visa.
 *
 * Field definitions mirror the Republic of Türkiye Ministry of Foreign
 * Affairs e-Visa application at `https://www.evisa.gov.tr`. The portal
 * is a public e-Visa system (no account required for the form itself —
 * email-driven verification at the end). Schema is a high-fidelity
 * reconstruction from the public landing pages, the e-Visa eligibility
 * list, and consular guidance. Same posture as JP_TOURIST, EG_E_VISA,
 * TH_TOURIST_E_VISA, MY_TOURIST_E_VISA.
 *
 * Scope: Tourist e-Visa only — single-entry (~USD 50, 180-day validity,
 * 30 or 90-day max stay depending on nationality) and multiple-entry
 * (~USD 80, 180-day validity, 30 or 90-day max stay per entry).
 * Variant captured by `visa_type_requested`. Sub-purpose tourism / commerce
 * captured by `purpose_of_visit`.
 *
 * Out of scope: Work Permit (separate Çalışma İzni system), Student
 * Visa (consular flow), Humanitarian Residence Permit, Long-term
 * Residence Permit, Family Residence Permit, Citizenship by Investment,
 * and visa-on-arrival (eliminated for most nationalities post-2013).
 *
...
const HAS_HOST_IN_TR = "has_host_in_turkiye === yes";
const VISITED_TR_BEFORE = "visited_turkiye_before === yes";
const REFUSED_VISA_TR = "refused_visa_or_entry_turkiye === yes";
...
// Türkiye e-Visa supports tourism + commerce (business visit) under one form.
const PURPOSE_OF_VISIT_OPTIONS = [
  { value: "tourism", text: "Tourism" },
  { value: "commerce", text: "Commerce / Business visit" },
];

const VISA_TYPE_REQUESTED_OPTIONS = [
  { value: "single", text: "Tourist e-Visa — Single entry, 180-day validity, 30 or 90-day stay, ~USD 50" },
  { value: "multiple", text: "Tourist e-Visa — Multiple entry, 180-day validity, 30 or 90-day stay per entry, ~USD 80" },
];

const PORT_OF_ENTRY_OPTIONS = [
  { value: "istanbul_ist", text: "Istanbul Airport (IST)" },
  { value: "istanbul_saw", text: "Istanbul Sabiha Gökçen Airport (SAW)" },
  { value: "ankara_esb", text: "Ankara Esenboğa Airport (ESB)" },
  { value: "antalya_ayt", text: "Antalya Airport (AYT)" },
  { value: "izmir_adb", text: "İzmir Adnan Menderes Airport (ADB)" },
  { value: "bodrum_bjv", text: "Bodrum Milas Airport (BJV)" },
  { value: "dalaman_dlm", text: "Dalaman Airport (DLM)" },
  { value: "trabzon_tzx", text: "Trabzon Airport (TZX)" },
  { value: "kapikule_land", text: "Kapıkule (Bulgaria border)" },
  { value: "ipsala_land", text: "İpsala (Greece border)" },
  { value: "sarp_land", text: "Sarp (Georgia border)" },
  { value: "habur_land", text: "Habur (Iraq border)" },
  { value: "istanbul_seaport", text: "Istanbul Galataport / Karaköy (cruise)" },
  { value: "kusadasi_seaport", text: "Kuşadası (cruise)" },
  { value: "izmir_seaport", text: "İzmir Alsancak Port" },
  { value: "other", text: "Other" },
];

const ACCOMMODATION_TYPE_OPTIONS = [
  { value: "hotel", text: "Hotel" },
  { value: "resort", text: "Resort / All-inclusive" },
  { value: "rental_apartment", text: "Rental apartment" },
  { value: "host_residence", text: "Residence of host (friend or relative)" },
  { value: "guesthouse", text: "Hostel / Pansiyon" },
  { value: "other", text: "Other" },
];

const EXPENSE_BEARER_OPTIONS = [{ value: "self", text: "Self" }, { value: "employer", text: "Employer" }, { value: "family", text: "Family member" }, { value: "host", text: "Host in Türkiye" }, { value: "other", text: "Other" }];
...
const FIELDS: FieldDef[] = [
  ...
  { field_name: "passport_expiry_date", label: "Date of expiry (must be valid 60+ days beyond intended departure from Türkiye)", field_type: "date", required: true, step_number: 2, step_name: "Passport", display_order: 5, validation_rules: { format: "DD/MM/YYYY", inline_group: "passport_dates" } },
  ...
  { field_name: "purpose_of_visit", label: "Purpose of visit to Türkiye", field_type: "select", required: true, step_number: 5, step_name: "Trip Details", display_order: 2, options: PURPOSE_OF_VISIT_OPTIONS },
  { field_name: "intended_arrival_date", label: "Intended date of arrival in Türkiye", field_type: "date", required: true, step_number: 5, step_name: "Trip Details", display_order: 3, validation_rules: { format: "DD/MM/YYYY", inline_group: "trip_dates" } },
  { field_name: "intended_length_of_stay", label: "Intended length of stay (days, max 90)", field_type: "text", required: true, step_number: 5, step_name: "Trip Details", display_order: 4, placeholder: "e.g. 14", validation_rules: { pattern: "^(?:[1-9]|[1-8][0-9]|90)$", inline_group: "trip_dates" } },
  ...
  { field_name: "carrier_name", label: "Name of airline, ship, or transport carrier", field_type: "text", required: false, step_number: 5, step_name: "Trip Details", display_order: 7, placeholder: "e.g. Turkish Airlines, Pegasus", validation_rules: { maxLength: 80 } },
  { field_name: "accommodation_type", label: "Type of accommodation in Türkiye", field_type: "select", required: true, step_number: 5, step_name: "Trip Details", display_order: 8, options: ACCOMMODATION_TYPE_OPTIONS },
  ...
  { field_name: "accommodation_address", label: "Address in Türkiye", field_type: "text", required: true, step_number: 5, step_name: "Trip Details", display_order: 10, validation_rules: { maxLength: 200, block_group: "accommodation_details" } },
  { field_name: "accommodation_city", label: "City / Province in Türkiye", field_type: "text", required: true, step_number: 5, step_name: "Trip Details", display_order: 11, validation_rules: { maxLength: 80, block_group: "accommodation_details" } },
  ...
  // STEP 6: Host
  { field_name: "has_host_in_turkiye", label: "Do you have a host (friend, relative, or sponsor) in Türkiye?", field_type: "radio", required: true, step_number: 6, step_name: "Host in Türkiye", display_order: 1, options: YES_NO },
  { field_name: "host_full_name", label: "Host — Full name", field_type: "text", required: true, step_number: 6, step_name: "Host in Türkiye", display_order: 2, conditional_logic: { showIf: HAS_HOST_IN_TR }, validation_rules: { maxLength: 120, block_group: "host" } },
  { field_name: "host_relationship_to_applicant", label: "Host — Relationship to applicant", field_type: "text", required: true, step_number: 6, step_name: "Host in Türkiye", display_order: 3, conditional_logic: { showIf: HAS_HOST_IN_TR }, validation_rules: { maxLength: 80, block_group: "host" } },
  { field_name: "host_address", label: "Host — Address in Türkiye", field_type: "text", required: true, step_number: 6, step_name: "Host in Türkiye", display_order: 4, conditional_logic: { showIf: HAS_HOST_IN_TR }, validation_rules: { maxLength: 200, block_group: "host" } },
  { field_name: "host_phone", label: "Host — Telephone (incl. country code)", field_type: "text", required: true, step_number: 6, step_name: "Host in Türkiye", display_order: 5, conditional_logic: { showIf: HAS_HOST_IN_TR }, validation_rules: { maxLength: 30, block_group: "host" } },

  // STEP 7: Travel History
  { field_name: "visited_turkiye_before", label: "Have you ever visited Türkiye before?", field_type: "radio", required: true, step_number: 7, step_name: "Travel History", display_order: 1, options: YES_NO },
  { field_name: "prior_tr_visit_arrival_date", label: "Prior Türkiye visit — Arrival date", field_type: "date", required: true, step_number: 7, step_name: "Travel History", display_order: 2, conditional_logic: { showIf: VISITED_TR_BEFORE }, validation_rules: { format: "DD/MM/YYYY", repeatable: true, repeat_group: "prior_tr_visits", max_items: 5 } },
  { field_name: "prior_tr_visit_departure_date", label: "Prior Türkiye visit — Departure date", field_type: "date", required: true, step_number: 7, step_name: "Travel History", display_order: 3, conditional_logic: { showIf: VISITED_TR_BEFORE }, validation_rules: { format: "DD/MM/YYYY", repeatable: true, repeat_group: "prior_tr_visits" } },
  { field_name: "prior_tr_visit_purpose", label: "Prior Türkiye visit — Purpose", field_type: "text", required: true, step_number: 7, step_name: "Travel History", display_order: 4, conditional_logic: { showIf: VISITED_TR_BEFORE }, validation_rules: { maxLength: 120, repeatable: true, repeat_group: "prior_tr_visits" } },
  { field_name: "refused_visa_or_entry_turkiye", label: "Have you ever been refused a visa to, or denied entry into, Türkiye?", field_type: "radio", required: true, step_number: 7, step_name: "Travel History", display_order: 5, options: YES_NO },
  { field_name: "refused_visa_tr_details", label: "Provide details (date, place, reason)", field_type: "textarea", required: true, step_number: 7, step_name: "Travel History", display_order: 6, conditional_logic: { showIf: REFUSED_VISA_TR }, validation_rules: { maxLength: 1000 } },
  ...
  { field_name: "has_been_deported", label: "Have you ever been deported from Türkiye or any other country?", field_type: "radio", required: true, step_number: 8, step_name: "Character & Declaration", display_order: 3, options: YES_NO },
  ...
  { field_name: "final_declaration", label: "I hereby declare that the statements made in this application are true and correct, and I understand that any false statement may result in refusal of the visa or denial of entry into the Republic of Türkiye.", field_type: "checkbox", required: true, step_number: 8, step_name: "Character & Declaration", display_order: 8, options: [{ value: "yes", text: "I agree" }] },
];
