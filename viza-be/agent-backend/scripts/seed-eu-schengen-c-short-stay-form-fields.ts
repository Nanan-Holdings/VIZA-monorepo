...
const FIELDS: FieldDef[] = [
  ...
  { field_name: "phone_number", label: "Telephone number (including country code)", field_type: "text", required: true, step_number: 5, step_name: "Contact Details & Residence", display_order: 7, validation_rules: { maxLength: 20 } },
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  { field_name: "current_occupation", label: "Current occupation", field_type: "text", required: true, step_number: 6, step_name: "Occupation", display_order: 1, placeholder: "e.g., Software engineer", validation_rules: { maxLength: 80, pattern: "^[A-Za-z0-9 .,'&/()\\-]+$", official_portal: "france_visas", official_rule: "If mapped to France-Visas occupation Other, the official portal also requires an Other occupation description and employer/school details." } },
  ...
  { field_name: "fv_business_segment", label: "France-Visas sector", field_type: "select", required: true, step_number: 6, step_name: "Occupation", display_order: 3, conditional_logic: { showIf: "is_student === no" }, options: [
    { value: "OTH", text: "Other activities", label_zh: "其他活动", label_en: "Other activities" },
    { value: "J", text: "Information and communication", label_zh: "信息和通信", label_en: "Information and communication" },
  ...
  { field_name: "employer_city", label: "Employer city", field_type: "text", required: true, step_number: 6, step_name: "Occupation", display_order: 5, conditional_logic: { showIf: "is_student === no" }, validation_rules: { maxLength: 50, pattern: "^[A-Za-z .'-]+$", block_group: "employer" } },
  ...
  { field_name: "employer_phone", label: "Employer telephone number", field_type: "text", required: true, step_number: 6, step_name: "Occupation", display_order: 7, conditional_logic: { showIf: "is_student === no" }, validation_rules: { maxLength: 20, pattern: "^[+0-9][0-9 .()\\-]{5,19}$", block_group: "employer" } },
  { field_name: "employer_email", label: "Employer email address", field_type: "text", required: false, step_number: 6, step_name: "Occupation", display_order: 8, conditional_logic: { showIf: "is_student === no" }, placeholder: "e.g., hr@example.com", validation_rules: { maxLength: 70, pattern: "^[^\\s@]+@[^\\s@]+\\.[^\\s@]+$", block_group: "employer", official_note: "France-Visas requires either employer phone or employer email; VIZA asks phone as required and email as optional backup." } },
  ...
  //
  // The first three entries below are needed by France-Visas step 3 ("Your
  // last visa"): a gate radio for whether the applicant held a Schengen
  // visa in the last 5 years, plus the validity range. These are also
  // legitimate Annex I prior-visa data — France keeps them as separate
  // fields whereas the EU consolidation rolled them into the fingerprint
  // block. The prev_schengen_fingerprints_* trio that follows acts as the
  // second-level "fingerprints captured?" branch within step 3.
  // ═══════════════════════════════════════════════════════════════════════════
  { field_name: "prior_schengen_visa_5y", label: "Have you held a Schengen visa in the last 5 years?", field_type: "radio", required: true, step_number: 10, step_name: "Travel History", display_order: 1, options: YES_NO },
  { field_name: "prior_schengen_visa_valid_from", label: "Last Schengen visa — valid from", field_type: "date", required: true, step_number: 10, step_name: "Travel History", display_order: 2, conditional_logic: { showIf: "prior_schengen_visa_5y === yes" }, validation_rules: { format: "DD/MM/YYYY", inline_group: "prior_visa_dates", block_group: "prior_visa" } },
  { field_name: "prior_schengen_visa_valid_to", label: "Last Schengen visa — valid until", field_type: "date", required: true, step_number: 10, step_name: "Travel History", display_order: 3, conditional_logic: { showIf: "prior_schengen_visa_5y === yes" }, validation_rules: { format: "DD/MM/YYYY", inline_group: "prior_visa_dates", block_group: "prior_visa" } },
  { field_name: "prev_schengen_fingerprints_given", label: "Have your fingerprints been collected previously for the purpose of applying for a Schengen visa?", field_type: "radio", required: true, step_number: 10, step_name: "Travel History", display_order: 4, conditional_logic: { showIf: "prior_schengen_visa_5y === yes" }, options: YES_NO },
  { field_name: "prev_fingerprints_date", label: "Date fingerprints were collected (if known)", field_type: "date", required: false, step_number: 10, step_name: "Travel History", display_order: 5, conditional_logic: { showIf: "prev_schengen_fingerprints_given === yes" }, validation_rules: { format: "DD/MM/YYYY" } },
  { field_name: "prev_fingerprints_visa_sticker", label: "Number of the visa (if known)", field_type: "text", required: false, step_number: 10, step_name: "Travel History", display_order: 6, conditional_logic: { showIf: "prev_schengen_fingerprints_given === yes" }, validation_rules: { maxLength: 30 } },
  { field_name: "has_entry_permit_final_destination", label: "Do you hold an entry permit for the final country of destination (where applicable)?", field_type: "radio", required: true, step_number: 10, step_name: "Travel History", display_order: 7, options: YES_NO },
  { field_name: "entry_permit_issuing_authority", label: "Entry permit — issued by", field_type: "text", required: true, step_number: 10, step_name: "Travel History", display_order: 8, conditional_logic: { showIf: "has_entry_permit_final_destination === yes" }, validation_rules: { maxLength: 100, block_group: "entry_permit" } },
  { field_name: "entry_permit_valid_from", label: "Entry permit — valid from", field_type: "date", required: true, step_number: 10, step_name: "Travel History", display_order: 9, conditional_logic: { showIf: "has_entry_permit_final_destination === yes" }, validation_rules: { format: "DD/MM/YYYY", inline_group: "entry_permit_dates", block_group: "entry_permit" } },
  { field_name: "entry_permit_valid_until", label: "Entry permit — valid until", field_type: "date", required: true, step_number: 10, step_name: "Travel History", display_order: 10, conditional_logic: { showIf: "has_entry_permit_final_destination === yes" }, validation_rules: { format: "DD/MM/YYYY", inline_group: "entry_permit_dates", block_group: "entry_permit" } },
  { field_name: "ever_refused_schengen_visa", label: "Have you ever been refused a Schengen visa?", field_type: "radio", required: true, step_number: 10, step_name: "Travel History", display_order: 11, options: YES_NO },
  { field_name: "ever_refused_schengen_visa_details", label: "Please provide details of the refusal", field_type: "textarea", required: true, step_number: 10, step_name: "Travel History", display_order: 12, conditional_logic: { showIf: "ever_refused_schengen_visa === yes" }, validation_rules: { maxLength: 500 } },
