...
const PASSENGER_STEP = "旅客信息";
const TRIP_STEP = "行程信息";
const REVIEW_STEP = "确认申报";
...
export const VN_PREARRIVAL_FORM_FIELDS: VnPrearrivalFieldDef[] = [
  ...
  { field_name: "passport_type", label: "Passport Type", field_type: "select", required: true, step_number: 1, step_name: PASSENGER_STEP, display_order: 2, options: VN_PREARRIVAL_PASSPORT_TYPE_OPTIONS, validation_rules: rules("护照类型", { official: true, official_source: "prearrival_category:passport_type" }) },
  ...
  { field_name: "phone_number", label: "Phone Number", field_type: "text", required: true, step_number: 1, step_name: PASSENGER_STEP, display_order: 11, validation_rules: rules("电话号码", { official: true, pattern: "^[0-9]{6,15}$" }) },
  {
  ...
  { field_name: "visa_number", label: "Number", field_type: "text", required: true, step_number: 1, step_name: PASSENGER_STEP, display_order: 15, validation_rules: rules("编号", { official: true, maxLength: 64, helper_zh: "电子签证编号通常显示在 e-visa PDF 顶部的 Registration code / No. 一栏，请按官方文件原样填写。", helper_en: "For an e-visa, find this number near the top of the e-visa PDF in the Registration code / No. area and enter it exactly as shown." }) },
  ...
  { field_name: "visa_expiry_date", label: "Date of Expiry (DD/MM/YYYY)", field_type: "date", required: true, step_number: 1, step_name: PASSENGER_STEP, display_order: 17, validation_rules: rules("有效期至（DD/MM/YYYY）", { official: true }) },
  { field_name: "visa_issued_place", label: "Issued Place", field_type: "select", required: false, step_number: 1, step_name: PASSENGER_STEP, display_order: 18, validation_rules: rules("签发地点", { official: true, official_source: "prearrival_category:visa_issue_place", remote_search: true }) },
  ...
  { field_name: "mode_of_travel", label: "Mode of Travel", field_type: "radio", required: true, step_number: 2, step_name: TRIP_STEP, display_order: 3, options: VN_PREARRIVAL_TRAVEL_MODE_OPTIONS, validation_rules: rules("旅行方式", { official: true }) },
  {
  ...
  { field_name: "vehicle_identification_number", label: "Vehicle identification number", field_type: "text", required: true, step_number: 2, step_name: TRIP_STEP, display_order: 6, conditional_logic: showIf("mode_of_travel !== air"), validation_rules: rules("交通工具识别编号", { official: true, maxLength: 40 }) },
  { field_name: "land_border_gate", label: "Border Gate", field_type: "select", required: true, step_number: 2, step_name: TRIP_STEP, display_order: 7, options: VN_PREARRIVAL_BORDER_GATE_OPTIONS, conditional_logic: showIf("mode_of_travel === land"), validation_rules: rules("陆路口岸", { official: true, official_source: "prearrival_category:border_gate" }) },
  { field_name: "sea_port", label: "Border Gate", field_type: "select", required: true, step_number: 2, step_name: TRIP_STEP, display_order: 8, options: VN_PREARRIVAL_PORT_OPTIONS, conditional_logic: showIf("mode_of_travel === sea"), validation_rules: rules("海港口岸", { official: true, official_source: "prearrival_category:port" }) },
  { field_name: "accommodation_type", label: "Type of Accommodation in Vietnam", field_type: "radio", required: true, step_number: 2, step_name: TRIP_STEP, display_order: 9, options: VN_PREARRIVAL_ACCOMMODATION_OPTIONS, validation_rules: rules("在越南住宿类型", { official: true }) },
  { field_name: "province_city_of_hotel", label: "Province / City of Hotel", field_type: "select", required: true, step_number: 2, step_name: TRIP_STEP, display_order: 10, conditional_logic: showIf("accommodation_type === hotel"), validation_rules: rules("酒店所在省 / 市", { official: true, official_source: "prearrival_category:administrative_unit_level1", remote_search: true }) },
  { field_name: "ward_commune_of_hotel", label: "Ward / Commune of Hotel", field_type: "select", required: true, step_number: 2, step_name: TRIP_STEP, display_order: 11, conditional_logic: showIf("accommodation_type === hotel"), validation_rules: rules("酒店所在坊 / 社", { official: true, official_source: "prearrival_category:administrative_unit_level2", remote_search: true, depends_on: "province_city_of_hotel" }) },
  { field_name: "hotel_accommodation_address", label: "Accommodation Address", field_type: "select", required: true, step_number: 2, step_name: TRIP_STEP, display_order: 12, conditional_logic: showIf("accommodation_type === hotel"), validation_rules: rules("住宿地址", { official: true, official_source: "prearrival_category:hotel", remote_search: true, depends_on: "ward_commune_of_hotel" }) },
  { field_name: "accommodation_address", label: "Accommodation Address", field_type: "text", required: true, step_number: 2, step_name: TRIP_STEP, display_order: 13, conditional_logic: showIf("accommodation_type in [residential, others]"), validation_rules: rules("住宿地址", { official: true, maxLength: 300 }) },
  { field_name: "workplace_information", label: "Workplace Information", field_type: "textarea", required: false, step_number: 2, step_name: TRIP_STEP, display_order: 14, validation_rules: rules("工作单位信息", { official: true, maxLength: 300 }) },
  { field_name: "departure_date_from_vietnam", label: "Date of departure from Vietnam (DD/MM/YYYY GMT+7)", field_type: "date", required: false, step_number: 2, step_name: TRIP_STEP, display_order: 15, validation_rules: rules("离开越南日期（DD/MM/YYYY GMT+7）", { official: true }) },
