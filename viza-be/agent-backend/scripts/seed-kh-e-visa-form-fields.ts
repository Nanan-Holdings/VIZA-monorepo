/**
 * Seed script: visa_form_fields for Cambodia Tourist e-Visa.
 *
 * Field definitions mirror the Royal Government of Cambodia Ministry
 * of Foreign Affairs and International Cooperation e-Visa application
 * at `https://www.evisa.gov.kh`. The portal is publicly accessible
 * (no account required for the form). Schema is a high-fidelity
 * reconstruction from public landing pages and the General Department
 * of Immigration applicant guidance.
 *
 * Scope: Tourist e-Visa only — single-entry (~USD 36 incl. service fee,
 * 3-month validity, 30-day stay). Variant captured by
 * `visa_type_requested` (single value reserved for future expansion).
 *
 * Out of scope: Business e-Visa (different evidence pack), Ordinary
 * Visa categories applied at consular posts (E-Class for work, K-Class
 * for foreign dependants of Cambodian citizens, etc.), and visa-on-
 * arrival (most nationalities, USD 30 cash at major entry points).
 *
 * Document uploads (passport bio, photo) out-of-schema per playbook §5.6.
 *
 * Run: npx tsx scripts/seed-kh-e-visa-form-fields.ts
 */
...
const VISA_TYPE = "KH_TOURIST_E_VISA";
...
const HAS_HOST_IN_KH = "has_host_in_cambodia === yes";
const VISITED_KH_BEFORE = "visited_cambodia_before === yes";
const REFUSED_VISA_KH = "refused_visa_or_entry_cambodia === yes";
...
const VISA_TYPE_REQUESTED_OPTIONS = [
  { value: "tourist_single", text: "Tourist e-Visa — Single entry, 3-month validity, 30-day stay, ~USD 36" },
];

const PORT_OF_ENTRY_OPTIONS = [
  { value: "phnom_penh_pnh", text: "Phnom Penh International Airport (PNH)" },
  { value: "siem_reap_rep", text: "Siem Reap-Angkor International Airport (REP)" },
  { value: "sihanoukville_kos", text: "Sihanoukville International Airport (KOS)" },
  { value: "poipet_land", text: "Poipet Border (Thailand border)" },
  { value: "bavet_land", text: "Bavet Border (Vietnam border)" },
  { value: "cham_yeam_land", text: "Cham Yeam Border (Thailand border)" },
  { value: "dong_kralor_land", text: "Trapeang Sre / Dong Kralor (Laos border)" },
  { value: "phnom_den_land", text: "Phnom Den Border (Vietnam border, Takeo)" },
  { value: "sihanoukville_seaport", text: "Sihanoukville Autonomous Port (cruise / sea)" },
...
const ACCOMMODATION_TYPE_OPTIONS = [
  ...
  { value: "guesthouse", text: "Guesthouse / Hostel" },
  { value: "other", text: "Other" },
];

const EXPENSE_BEARER_OPTIONS = [{ value: "self", text: "Self" }, { value: "employer", text: "Employer" }, { value: "family", text: "Family member" }, { value: "host", text: "Host in Cambodia" }, { value: "other", text: "Other" }];
...
const FIELDS: FieldDef[] = [
  ...
  { field_name: "purpose_of_visit", label: "Purpose of visit to Cambodia", field_type: "select", required: true, step_number: 5, step_name: "Trip Details", display_order: 2, options: PURPOSE_OF_VISIT_OPTIONS },
  { field_name: "intended_arrival_date", label: "Intended date of arrival in Cambodia", field_type: "date", required: true, step_number: 5, step_name: "Trip Details", display_order: 3, validation_rules: { format: "DD/MM/YYYY", inline_group: "trip_dates" } },
  ...
  { field_name: "carrier_name", label: "Name of airline, ship, or transport carrier", field_type: "text", required: false, step_number: 5, step_name: "Trip Details", display_order: 7, placeholder: "e.g. Cambodia Angkor Air, Sky Angkor", validation_rules: { maxLength: 80 } },
  { field_name: "accommodation_type", label: "Type of accommodation in Cambodia", field_type: "select", required: true, step_number: 5, step_name: "Trip Details", display_order: 8, options: ACCOMMODATION_TYPE_OPTIONS },
  ...
  { field_name: "accommodation_address", label: "Address in Cambodia", field_type: "text", required: true, step_number: 5, step_name: "Trip Details", display_order: 10, validation_rules: { maxLength: 200, block_group: "accommodation_details" } },
  { field_name: "accommodation_city", label: "City / Province in Cambodia", field_type: "text", required: true, step_number: 5, step_name: "Trip Details", display_order: 11, validation_rules: { maxLength: 80, block_group: "accommodation_details" } },
  ...
  { field_name: "has_host_in_cambodia", label: "Do you have a host (friend, relative, or sponsor) in Cambodia?", field_type: "radio", required: true, step_number: 6, step_name: "Host in Cambodia", display_order: 1, options: YES_NO },
  { field_name: "host_full_name", label: "Host — Full name", field_type: "text", required: true, step_number: 6, step_name: "Host in Cambodia", display_order: 2, conditional_logic: { showIf: HAS_HOST_IN_KH }, validation_rules: { maxLength: 120, block_group: "host" } },
  { field_name: "host_relationship_to_applicant", label: "Host — Relationship to applicant", field_type: "text", required: true, step_number: 6, step_name: "Host in Cambodia", display_order: 3, conditional_logic: { showIf: HAS_HOST_IN_KH }, validation_rules: { maxLength: 80, block_group: "host" } },
  { field_name: "host_address", label: "Host — Address in Cambodia", field_type: "text", required: true, step_number: 6, step_name: "Host in Cambodia", display_order: 4, conditional_logic: { showIf: HAS_HOST_IN_KH }, validation_rules: { maxLength: 200, block_group: "host" } },
  { field_name: "host_phone", label: "Host — Telephone (incl. country code)", field_type: "text", required: true, step_number: 6, step_name: "Host in Cambodia", display_order: 5, conditional_logic: { showIf: HAS_HOST_IN_KH }, validation_rules: { maxLength: 30, block_group: "host" } },

  // STEP 7: Travel History
  { field_name: "visited_cambodia_before", label: "Have you ever visited Cambodia before?", field_type: "radio", required: true, step_number: 7, step_name: "Travel History", display_order: 1, options: YES_NO },
  { field_name: "prior_kh_visit_arrival_date", label: "Prior Cambodia visit — Arrival date", field_type: "date", required: true, step_number: 7, step_name: "Travel History", display_order: 2, conditional_logic: { showIf: VISITED_KH_BEFORE }, validation_rules: { format: "DD/MM/YYYY", repeatable: true, repeat_group: "prior_kh_visits", max_items: 5 } },
  { field_name: "prior_kh_visit_departure_date", label: "Prior Cambodia visit — Departure date", field_type: "date", required: true, step_number: 7, step_name: "Travel History", display_order: 3, conditional_logic: { showIf: VISITED_KH_BEFORE }, validation_rules: { format: "DD/MM/YYYY", repeatable: true, repeat_group: "prior_kh_visits" } },
  { field_name: "prior_kh_visit_purpose", label: "Prior Cambodia visit — Purpose", field_type: "text", required: true, step_number: 7, step_name: "Travel History", display_order: 4, conditional_logic: { showIf: VISITED_KH_BEFORE }, validation_rules: { maxLength: 120, repeatable: true, repeat_group: "prior_kh_visits" } },
  { field_name: "refused_visa_or_entry_cambodia", label: "Have you ever been refused a visa to, or denied entry into, Cambodia?", field_type: "radio", required: true, step_number: 7, step_name: "Travel History", display_order: 5, options: YES_NO },
  { field_name: "refused_visa_kh_details", label: "Provide details", field_type: "textarea", required: true, step_number: 7, step_name: "Travel History", display_order: 6, conditional_logic: { showIf: REFUSED_VISA_KH }, validation_rules: { maxLength: 1000 } },
  ...
  { field_name: "has_been_deported", label: "Have you ever been deported from Cambodia or any other country?", field_type: "radio", required: true, step_number: 8, step_name: "Character & Declaration", display_order: 3, options: YES_NO },
  ...
  { field_name: "final_declaration", label: "I hereby declare that the statements made in this application are true and correct, and I understand that any false statement may result in refusal of the visa or denial of entry into the Kingdom of Cambodia.", field_type: "checkbox", required: true, step_number: 8, step_name: "Character & Declaration", display_order: 8, options: [{ value: "yes", text: "I agree" }] },
];
