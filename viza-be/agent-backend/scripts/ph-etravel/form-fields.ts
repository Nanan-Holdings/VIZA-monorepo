...
export const PH_ETRAVEL_SEA_FLOW_CLIENT_WIRING_E24 = {
  ...
  with_custom_declaration: {
    ...
    port_to_manual_or_electronic_mapping: "verified_live_path_correlation_requires_page_content_confirmation",
    route_selection: "metadata_hint_then_verify_rendered_page_title_content",
  },
} as const;
...
export const PH_ETRAVEL_GENERAL_DECLARATION_ITEM_IDS = PH_ETRAVEL_DECLARATION_CHECKLIST
  .filter((item) => item.type !== "CURRENCY")
  .map((item) => item.id);
export const PH_ETRAVEL_GENERAL_DECLARATION_POSITIVE_AMOUNT_RULE = {
  scope: "AIR General Declaration only",
  when: "amount_of_goods_acquired.amount > 0",
  requires_any_yes: PH_ETRAVEL_GENERAL_DECLARATION_ITEM_IDS.map((itemId) => `customs_checklist_${itemId}`),
  excludes_currency_threshold_questions: ["customs_checklist_1", "customs_checklist_2"],
  evidence: "E42_confirmed_user_provided_behavior",
  server_acceptance: "needs_review",
} as const;
...
const GOODS_DETAIL_FIELDS: PhEtravelFieldDef[] = [
  { field_name: "goods_total_currency", label: "Currency of Total Value of Goods", field_type: "select", required: false, step_number: 7, step_name: "Customs Declaration", display_order: 30, options: PH_ETRAVEL_CURRENCY_TYPE_OPTIONS, conditional_logic: showIf(HAS_CUSTOMS_DECLARATION), validation_rules: rules("申报物品总价值币种", { official: true, official_key: "amount_of_goods_acquired.currency", official_control_type: "radio", customs_contract: "goods_total_value", structured_contract: "amount_of_goods_acquired", currency_switch_contract: "Philippine_Peso_and_US_Dollar_use_the_same_amount_control", selector_evidence_level: "E42_confirmed_user_provided_behavior", evidence_level: "needs_review_requiredness", ...CANONICAL_OPTION_RULES }) },
  { field_name: "goods_total_amount", label: "Total Value of Goods", field_type: "text", required: false, step_number: 7, step_name: "Customs Declaration", display_order: 31, conditional_logic: showIf(HAS_CUSTOMS_DECLARATION), validation_rules: rules("申报物品总价值", { official: true, official_key: "amount_of_goods_acquired.amount", official_control_type: "text", customs_contract: "goods_total_value", structured_contract: "amount_of_goods_acquired", initial_rendered_value: "0", positive_amount_rule: PH_ETRAVEL_GENERAL_DECLARATION_POSITIVE_AMOUNT_RULE, selector_evidence_level: "E42_confirmed_user_provided_behavior", evidence_level: "needs_review_requiredness", pattern: "^[0-9]+(\\.[0-9]{1,2})?$" }) },
  ...PH_ETRAVEL_GENERAL_DECLARATION_ITEM_IDS.flatMap((itemId, itemIndex): PhEtravelFieldDef[] => {
    const itemCondition = `${HAS_CUSTOMS_DECLARATION} && customs_checklist_${itemId} === yes`;
    const repeatGroup = `customs_checklist_${itemId}_items`;
    const itemRules = {
      official: true,
      official_group_key: "baggage.items[]",
      official_question_key: `check_lists.${itemId - 1}.response`,
      question_id: itemId,
      customs_contract: "question_specific_goods_item_detail",
      modal_behavior: "Add Item repeat row",
      repeatable: true,
      repeat_group: repeatGroup,
      repeat_contract: "baggage.items[]",
      official_table: "Quantity / Description / Amount in USD / Action",
      selector_evidence_level: "E42_confirmed_user_provided_AIR; SEA_question_specific_grouping_needs_review",
      evidence_level: "needs_review_requiredness",
      page_level_no_row_blocking: "needs_review_not_reproduced_after_delete",
    };
    const displayOrder = 32 + itemIndex * 3;
    return [
      { field_name: `customs_checklist_${itemId}_item_description`, label: `Question ${itemId} Item Description`, field_type: "textarea", required: false, step_number: 7, step_name: "Customs Declaration", display_order: displayOrder, conditional_logic: showIf(itemCondition), validation_rules: rules(`海关申报项目 ${itemId} 明细描述`, { ...itemRules, official_selector: "description", official_row_label: "Description", official_control_type: "textarea", empty_validation_observed: "Description Required", maxLength: 240 }) },
      { field_name: `customs_checklist_${itemId}_item_quantity`, label: `Question ${itemId} Item Quantity`, field_type: "text", required: false, step_number: 7, step_name: "Customs Declaration", display_order: displayOrder + 1, conditional_logic: showIf(itemCondition), validation_rules: rules(`海关申报项目 ${itemId} 明细数量`, { ...itemRules, official_selector: "quantity", official_row_label: "Quantity", official_control_type: "text", empty_validation_observed: "Quantity Required", pattern: "^[0-9]+$" }) },
      { field_name: `customs_checklist_${itemId}_item_amount_usd`, label: `Question ${itemId} Item Amount in USD`, field_type: "text", required: false, step_number: 7, step_name: "Customs Declaration", display_order: displayOrder + 2, conditional_logic: showIf(itemCondition), validation_rules: rules(`海关申报项目 ${itemId} 明细美元金额`, { ...itemRules, official_selector: "amount", official_row_label: "Amount in USD", official_control_type: "text", empty_validation_observed: "Amount in USD Required", pattern: "^[0-9]+(\\.[0-9]{1,2})?$" }) },
    ];
  }),
];

const CURRENCY_DETAIL_FIELDS: PhEtravelFieldDef[] = [
  { field_name: "currency_owner_not_applicable", label: "Please check if NOT APPLICABLE", field_type: "checkbox", required: false, step_number: 7, step_name: "Customs Declaration", display_order: 40, conditional_logic: showIf(HAS_CURRENCY_DECLARATION), validation_rules: rules("货币所有人信息不适用", { official: true, official_key: "owner_details_not_applicable", official_control_type: "checkbox", customs_contract: "currency_positive_detail", selector_evidence_level: "confirmed_live_E45", e45_empty_state_toggle: "direct_owner_inputs_disabled; country_combobox_native_disabled_not_observed", evidence_level: "needs_review_requiredness" }) },
  ...
  { field_name: "currency_source_other", label: "Other Source of Currency", field_type: "text", required: false, step_number: 7, step_name: "Customs Declaration", display_order: 64, conditional_logic: showIf(HAS_OTHER_CURRENCY_SOURCE), validation_rules: rules("其他货币来源", { official: true, official_key: "currency_source_other", official_control_type: "text", customs_contract: "currency_source_array_other", empty_validation_observed: "Required", validated_required_when: "currency_source includes OTHER", e45_live_other_child_visibility: "not_rendered_current_build; E7_wiring_conflict_needs_review", selector_evidence_level: "confirmed_live_visible_air_positive", evidence_level: "needs_review_requiredness", maxLength: 160 }) },
...
export const PH_ETRAVEL_FORM_FIELDS: PhEtravelFieldDef[] = [
  ...
  { field_name: "purpose_of_travel", label: "Purpose of Travel", field_type: "select", required: true, step_number: 3, step_name: "Travel Details - Philippine Arrival", display_order: 1, options: PH_ETRAVEL_PURPOSE_OPTIONS, validation_rules: rules("旅行目的", { official: true, official_key: "purpose_of_visit_code", dynamic_option_source: PH_ETRAVEL_DYNAMIC_OPTION_SOURCES.arrival_purpose_of_visits, current_arrival_option_source: "E49 UI-shaped /api/v1/common/purpose_of_visits current 16-row response", supersedes: "E46/E47 no-filter 15-row response", restored_current_options: ["POV999"], option_identity: "code", label_identity: "name", ...CANONICAL_OPTION_RULES }) },
  ...
  { field_name: "sea_port_of_entry", label: "Seaport of Destination in the Philippines", field_type: "select", required: false, step_number: 3, step_name: "Travel Details - Philippine Arrival", display_order: 17, options: PH_ETRAVEL_SEA_PORT_OPTIONS, conditional_logic: showIf(`${IS_SEA} && ${ARRIVAL_FLIGHT_TYPE}`), validation_rules: rules("菲律宾目的海港 / 入境海港", { official: true, official_key: "destination_port_code", transport_branch: "SEA", product_alias: true, distinct_from: "disembarking_port_code", dynamic_option_source: PH_ETRAVEL_DYNAMIC_OPTION_SOURCES.sea_destination_ports, option_identity: "code", label_identity: "name_not_unique", label_recovery_forbidden: true, duplicate_label_evidence: "E49 Port of Legazpi appears with two distinct official codes TP120 and LEGAZPI", port_metadata_field: "with_custom_declaration", port_metadata_contract: "official_page_branch_metadata_not_applicant_answer_or_requiredness", regular_page_gate: "registration.travel_port.with_custom_declaration", customs_hook_source_shape: "registration.with_custom_declaration", manual_electronic_mapping: "metadata_correlates_with_manual_vs_electronic_pages_but_runner_must_verify_rendered_page", route_selection: "metadata_hint_then_verify_rendered_page_title_content", client_requiredness: "public_bundle_only", server_requiredness: "needs_review", ...CANONICAL_OPTION_RULES, evidence_level: "verified_live_and_public_bundle_path_specific", observed_path: "E48 Basco Batanes/BSCBTN with_custom_declaration=0 manual route; E8/E9 Manila South Harbor/TP0103 with_custom_declaration=1 electronic route", option_value_evidence_level: "verified_public", selected_port_customs_flow_contract: "runtime_from_official_destination_port_metadata_plus_page_content_not_applicant_field" }) },
