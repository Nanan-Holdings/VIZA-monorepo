...
const confirm = process.env.ACCOUNT_ACTION_RLS_INITPLAN_DB_CONFIRM === "local-test";
const databaseUrl =
	process.env.ACCOUNT_ACTION_RLS_INITPLAN_DATABASE_URL ?? process.env.DATABASE_URL ?? "";
const marker = (process.env.ACCOUNT_ACTION_RLS_INITPLAN_DB_NONPRODUCTION ?? "").toLowerCase();
...
const migrationSql = readFileSync(
	fileURLToPath(new URL("../../drizzle/0178_account_action_log_rls_initplan.sql", import.meta.url)),
	"utf8",
);
...
const query = async <T extends Record<string, unknown> = Record<string, unknown>>(
	...
	if (!client) throw new Error("account-action RLS integration client is not initialized");
	return client.query<T>(sql, values);
};

describe.skipIf(!liveGateEnabled)("account-action-log RLS init-plan database integration", () => {
	beforeAll(async () => {
		...
		if (!allowedMarkers.has(databaseEnvironment)) {
			throw new Error(
				`Refusing account-action RLS integration DDL: database marker ${databaseEnvironment || "<unset>"} is not local/test`,
			);
		}
		...
		const collisions = await client.query<{ relation: string; present: string | null }>(`
			...
			FROM pg_catalog.unnest(ARRAY['applicant_profiles', 'account_action_log']) relation
		`);
		const present = collisions.rows.filter((row) => row.present).map((row) => row.relation);
		if (present.length > 0) {
			throw new Error(`Refusing account-action RLS fixture setup: relations exist: ${present.join(", ")}`);
		}
		...
		await client.query(`
			...
			GRANT SELECT ON public.applicant_profiles, public.account_action_log
		...
		const before = await client.query<{
			...
			WHERE policy.polrelid = 'public.account_action_log'::regclass
	...
	it("preserves the policy identity and exact two-path contract", async () => {
	...
	it("keeps both authenticated ownership paths, anon denial, and service visibility", async () => {
		...
		const owned = await query<{ label: string }>(
			"SELECT label FROM public.account_action_log ORDER BY id",
		);
		...
		expect((await query("SELECT id FROM public.account_action_log")).rows).toEqual([]);

		await query("RESET ROLE");
		await query("SET ROLE service_role");
		const serviceCount = await query<{ count: string }>(
			"SELECT count(*)::text AS count FROM public.account_action_log",
		);
